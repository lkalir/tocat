...
Counts bytes past its own position and, on reaching the limit, asks the host to
stop reading.
...
naming the limit and where the transfer actually ended, which differ under
`overshoot`:
...
Half a datagram is a corrupt message rather than a short read.

Below the limit nothing is copied: the chunk is passed through untouched, and
only the crossing one is ever materialised.
