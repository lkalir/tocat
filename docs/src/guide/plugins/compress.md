...
`flush` is on by default: whatever arrived is on the wire before the call
returns. Turn it off for bulk transfers where throughput matters and nobody is
waiting on a prompt, and output then appears only as zstd fills its own buffer
and at end of stream.

Both default to `detach`, since they are expensive enough per byte to be worth
their own task. `detach=false` is accepted if you would rather have them inline.

`compress` writes a zstd epilogue at end of stream; without it a peer's decoder
rejects the stream as truncated. Two paths never reach end of stream, a datagram
source and a held [`pipe`](../endpoints/pipe.md), so neither the epilogue nor
`report` happens there. Neither stage preserves message boundaries, so a
datagram destination on the same path draws a warning.
